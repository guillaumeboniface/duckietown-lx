...
def distance_traveled(ticks, encoder_resolution, wheel_radius):
     return ticks / encoder_resolution * wheel_radius * 2 * np.pi

def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    belief_in = belief

    left_distance = distance_traveled(left_encoder_ticks, robot_spec['encoder_resolution'], robot_spec['wheel_radius'])
    right_distance = distance_traveled(right_encoder_ticks, robot_spec['encoder_resolution'], robot_spec['wheel_radius'])
    delta_d = (left_distance + right_distance) / 2
    delta_phi = (right_distance - left_distance) / robot_spec['wheel_baseline']
    print(f"delta_d: {delta_d}, delta_phi: {delta_phi}")
    ...
    v = np.sin(grid_spec["phi"] + delta_phi / 2.0) * delta_d
    w = delta_phi
